...
namespace Admin.Api.UnitTests.Repositories
{
    public class PartnerUserRepositoryTests
    {
        ...
        public void AddPartnerUserAsync_Should_Generate_PartnerUserId_For_Insert()
        {
            ...
            var partnerUsers = Enumerable.Empty<Admin.Domain.Model.PartnerUser>().AsQueryable();
            ...
            partnerUserDbContextMock.Verify(
                repo => repo.AddAsync(It.Is<Admin.Domain.Model.PartnerUser>(p =>
...
*/
        ...
        private static Mock<PartnerUserDbContext> GetMockPartnerUserDbContext(IQueryable<Admin.Domain.Model.PartnerUser> partnerUsers)
        {
            var mockSet = new Mock<DbSet<Admin.Domain.Model.PartnerUser>>();

            mockSet.As<IAsyncEnumerable<Admin.Domain.Model.PartnerUser>>()
                .Setup(m => m.GetEnumerator())
                .Returns(new TestAsyncEnumerator<Admin.Domain.Model.PartnerUser>(partnerUsers.GetEnumerator()));

            mockSet.As<IQueryable<Admin.Domain.Model.PartnerUser>>()
                .Setup(m => m.Provider)
                .Returns(new TestAsyncQueryProvider<Admin.Domain.Model.PartnerUser>(partnerUsers.Provider));

            mockSet.As<IQueryable<Admin.Domain.Model.PartnerUser>>().Setup(m => m.Expression).Returns(partnerUsers.Expression);
            mockSet.As<IQueryable<Admin.Domain.Model.PartnerUser>>().Setup(m => m.ElementType).Returns(partnerUsers.ElementType);
            mockSet.As<IQueryable<Admin.Domain.Model.PartnerUser>>().Setup(m => m.GetEnumerator()).Returns(() => partnerUsers.GetEnumerator());
